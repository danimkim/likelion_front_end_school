...
function add(x, y=30){ // 3. add라는 이름의 함수에요. 근데 이 함수는 전달받아야할 값이 두개에요. 그래서 값 들어올 자리가 두개에요. 첫번째 자리 이름은 x, 두번째 자리 이름은 y에요.
	...
	return x + y;   // 7. return 뒤에는 이 함수가 호출되었을 때 내부 연산을 실행한 후 결과로 반환할 값 또는 표현식을 써줍니다. 이 함수의 경우 다른 표현식이 따로 없고 return x+y만 있는데
                  // 그러면 return 뒤의 표현식만 연산해서(매개변수x,y로 들어온 두 값 더하기) 나온 결과를 반환합니다.
